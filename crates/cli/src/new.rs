...
pub struct NewCommand {
  ...
  /// min node-api version
  min_node_api: u8,

  #[clap(short = 'l', long, default_value = "MIT")]
  license: String,
...
impl Executable for NewCommand {
  fn execute(&mut self) -> CommandResult {
    ...
    let default_name = path.iter().last().unwrap().to_string_lossy().to_string();
...
impl NewCommand {
  ...
  fn write_github_workflow(
    ...
    write_file(
      &format!("{}/.github/workflows/{file_name}", self.path),
      &github_workflow,
    )
  }
}
